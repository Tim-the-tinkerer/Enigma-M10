...
| Magic | Mode | Notes |
|-------|------|--------|
| `M10PW03` | Password | Opaque binary. Current password **write** path. |
| `M10PW02` | Password | Leftover decrypt. Public sizes; optional pad flags. |
| `M10PW01` | Password | Legacy decrypt. Public sizes, no padding. |
| `ENIGMAM10 vN` | External key, Internal key (and old JSON password files) | UTF-8 JSON ± binary region. Current **write** is **v6**. v1–v5 remain readable. |
...
u8    flags          bit0 name encrypted, bit2 folder, bits3–4 suite
                     (0=alpha36, 1=ascii, 2=base256). Other bits must be 0.
...
u8    salt[saltLen]
u8    nonce[16]
u16   nameLen        v6: 256 (Base-256) or 512 (Alpha-36 / ASCII-94)
u8    name[nameLen]  256-byte name field (u16be length + UTF-8 + random pad),
                     then pair-encoded for Alpha-36/ASCII-94, then optionally rotor-encrypted
...
## 2. External-key JSON (`ENIGMAM10 v6`)
...
| Field | Meaning |
|-------|---------|
| `version` | Must match header (`1`–`6`). New writes: **6**. |
| `kind` | `file` or `folder` |
| `cipherSuite` | `alpha36`, `ascii`, or `base256` |
...
Normal **Decrypt** accepts authenticated v2–v6 JSON and `M10PW01`–`M10PW03`. Relabeling v2+ as v1 is rejected on that path.
...
Files ≥ 1 MiB (and folder ZIPs of that size): zlib if it shrinks → dense pack → inner header + pad → M10, then binary after the JSON line. Limits: 2 GiB payload, 16 GiB archive. v6 ciphertext length is a multiple of 4096 (enforced on parse for `M10PW03` and ENIGMAM10 v6).
