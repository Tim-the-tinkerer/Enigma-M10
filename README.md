...
**Password (default).** A per-file salt and Argon2id (64 MiB / 3 / 1) derive the entire machine (rotors, reflector, notches, rings, positions, plugs, HMAC key). The archive is an opaque binary (`M10PW03`): salt, Argon2 parameters, nonce, encrypted name, HMAC, padded ciphertext. Exact sizes and CRC-32 are inside the ciphertext. No JSON, no rotors, no `.m10key`. Decrypt with the same password.
...
- **Suites** — Alpha-36, ASCII-94, Base-256 (default)
- **Streaming** — files ≥ 1 MiB pack from disk; up to 2 GiB payload / 16 GiB archive
- **Encrypt-then-MAC** on v3+ JSON and on `M10PW03` (header + ciphertext, before rotor decrypt)
...
**Password files** sniff as `M10PW03` (`M10PW02` / `M10PW01` still decrypt). **External-key files** sniff as `ENIGMAM10 v6` (JSON, optional binary region; v2–v5 still decrypt). EnigmaVault cannot open these files.
...
| File | What it is |
|------|------------|
| [FORMAT.md](FORMAT.md) | On-disk `M10PW03` and `ENIGMAM10 v6` (plus older decrypt) |
| [SECURITY.md](SECURITY.md) | Threat model; CRC vs HMAC; what this is not |
| [CHANGELOG.md](CHANGELOG.md) | App versions |

## Troubleshooting

- **“Not a valid archive”** on a password file — that copy of the app did not sniff `M10PW03`. Use this tree’s `EnigmaM10.app` (1.3.5+).
